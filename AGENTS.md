...
- `index.html` contains the semantic HTML, metadata, canonical/OG/Twitter tags, JSON-LD structured data, and page content.
- `assets/css/styles.css` contains the extracted site styling, responsive layout rules, skip-link/focus styles, and reduced-motion handling.
- `assets/js/main.js` contains progressive enhancement JavaScript for sticky header state, subtle pattern movement, and reveal animations with an IntersectionObserver fallback.
...
- Keep exactly one `<h1>` on the page; currently it is the hero heading `Herzlich willkommen`.
- Header and footer navigation point to in-page anchors only. There are no separate public HTML pages at the moment.
...
- The page includes a keyboard skip link pointing to `#main-content`; keep it working if the main layout changes.
...
- Current homepage photos include:
  - `fotos/urchenhof_front04_640x480.jpg` (hero and gallery image; also preloaded)
...
- Keep the above-the-fold hero image prioritized (`fetchpriority="high"` / preload) and lazy-load lower-page images where appropriate.
...
- Maintain a unique `<title>`, meta description, canonical URL, Open Graph tags, Twitter card, favicon, theme color, and JSON-LD structured data.
...
- Confirm local image references and local asset references exist before finishing.
...
- Basic validation command: use a small script or browser smoke test to confirm `index.html`, `assets/css/styles.css`, `assets/js/main.js`, local image references, exactly one `<h1>`, and `sitemap.xml` exist.
- Useful command for references: `rg -n "fotos/|<h1|<section|application/ld\+json|assets/css|assets/js" index.html`.
